...
def get_rp_env(args):
    ...
    env_type = 'atari'
    atari_envs = ['pong']
    if args.env_name in atari_envs:
        env = AtariEnv
        gym_env_kwargs = {'game': 'pong',
                          'obs_type': 'ram' # linear feature vector observation space
                          }
        time_limit = np.inf
    
    class RPAtariEnv(env):
        def __init__(self, args):
            super(RPAtariEnv, self).__init__(**gym_env_kwargs)
        ...
        def step(self, a):
            ...
            ob, reward, done, info = super(RPAtariEnv, self).step(a)

            # time limit
            self.nsteps += 1
            if self.nsteps >= time_limit:
                done = True
                # d
        ...
        def reset(self):
            ...
            self.first_pos = None
            self.nsteps = 0
            return super(RPAtariEnv, self).reset()

    return RPAtariEnv(args)
