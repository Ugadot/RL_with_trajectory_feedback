...
class RPDoublePendulumEnv(InvertedDoublePendulumEnv):
    def __init__(self, Pendulum_args):
        super(RPDoublePendulumEnv, self).__init__()
        
        self.tot_reward = 0
        self.tot_sparse_reward = 0
        self.nsteps = 0
        self.sparse_reward_value = Pendulum_args.sparse_reward_value
        self.use_sparse_reward = Pendulum_args.use_sparse_reward

        # distance reward vars
        self.reward_distance = Pendulum_args.reward_distance
        self.first_pos = None
        self.abs_first_pos = None
        
        # accu reward vars
        self.accumulative_threshold = Pendulum_args.accumulative_threshold
        self.temp_accu_reward = 0 
        
        # RP args (for online training)
        self.RP = None
        self.predict_with_action = Pendulum_args.predict_with_action

        if Pendulum_args.reward_pred or Pendulum_args.online_training:
            self.state_index = 0
            if Pendulum_args.online_training:
                load_path = None
            else:
                load_path = Pendulum_args.reward_pred
            ...
            self.RP = Predictor(threshold = 4,  # TODO: add to arguments?
                                ...
                                save_results_path = Pendulum_args.log_dir)
            
            self.rp_factor = Pendulum_args.rp_factor

        self.online_training = Pendulum_args.online_training
        self.device = Pendulum_args.device
        self._tot_steps = 0
        if self.online_training:
            self.replay_size = Pendulum_args.replay_size

            self.obs_buf = []
            self.r_buf = []

            self.step_count = 0
            self.win_size = Pendulum_args.win_size
            self._clear_current_buffer() #restart current cyclic buffers
            self.train_interval = Pendulum_args.train_interval
            self.model_var_thresh = Pendulum_args.var_threshold
            self._inserted_windows = 0
            self._asked_windows = 0
            
            #temp addition
            self._max_window_reward = -np.inf
            self._min_window_reward = np.inf
    ...
    def _show_window_images(self):
        ...
        print ("Window # " + str(self._inserted_windows) + "\t" + str(np.mean(nd_vars_buffer)))
        
        # --- Start multiprocessing for window 
        ...
        reward = _get_discrete_reward(self._max_window_reward * len(self.real_reward_buffer), self._min_window_reward * len(self.real_reward_buffer), 10, np.sum(nd_rewards_buffer))
        return reward  
    ...
    def step(self, a):
        ...
        #ob, reward, done, _ = super(RPDoublePendulumEnv, self).step(uniform_step)

        prev_ob = self._get_obs()

        ob, reward, done, _ = super(RPDoublePendulumEnv, self).step(a)
        ...
        if self.online_training:
            ...
            # temp addition (calc max and min widnow reawrd)
            current_window_reward = np.mean(np.array(self.real_reward_buffer))
            if current_window_reward and current_window_reward >= self._max_window_reward:
                self._max_window_reward = current_window_reward
            if current_window_reward and current_window_reward <= self._min_window_reward:
                self._min_window_reward = current_window_reward
    ...
    def reset_model(self):
        ...
        self.first_pos = None
        self.nsteps = 0
        return super(RPDoublePendulumEnv, self).reset_model()


def _get_discrete_reward(max_reward, min_reward, discrete_size, current_reward):
    dx = (max_reward - min_reward) / discrete_size
    current_discrete = int(round((current_reward - min_reward) / dx))
    #print (max_reward, min_reward, current_reward, current_discrete)
