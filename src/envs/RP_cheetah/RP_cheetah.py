...
class RPCheetahEnv(HalfCheetahEnv):
    def __init__(self, Cheetah_args):  
        
        super(RPCheetahEnv, self).__init__()

        self.tot_reward = 0
        self.tot_sparse_reward = 0
        self.nsteps = 0
        self.sparse_reward_value = Cheetah_args.sparse_reward_value
        self.use_sparse_reward = Cheetah_args.use_sparse_reward
        
        # distance reward vars
        self.reward_distance = Cheetah_args.reward_distance
        self.first_pos = None
        self.abs_first_pos = None
        
        # accu reward vars
        self.accumulative_threshold = Cheetah_args.accumulative_threshold
        self.temp_accu_reward = 0 
        
        # RP args (for online training)
        self.RP = None
        self.predict_with_action = Cheetah_args.predict_with_action

        if Cheetah_args.reward_pred or Cheetah_args.online_training:
            if Cheetah_args.online_training:
                load_path = None
            else:
                load_path = Cheetah_args.reward_pred
            ...
            self.RP = Predictor(threshold = 4,  # TODO: add to arguments?
                                ...
                                save_results_path = Cheetah_args.log_dir)
            
            self.rp_factor = Cheetah_args.rp_factor

        self.online_training = Cheetah_args.online_training
        self.device = Cheetah_args.device

        if self.online_training:
            self.replay_size = Cheetah_args.replay_size
            self.obs_buf = []
            self.r_buf = []
            self.done_buf = []
            self.step_count = 0
            self.win_size = Cheetah_args.win_size
            self.train_interval = Cheetah_args.train_interval
    ...
    def step(self, a):
        ...
        ob, reward, done, _ = super(RPCheetahEnv, self).step(a)
    ...
    def reset_model(self):
        ...
        self.tot_sparse_reward = 0
        self.nsteps = 0
        return super(RPCheetahEnv, self).reset_model()
