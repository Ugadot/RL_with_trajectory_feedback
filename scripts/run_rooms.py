...
def run(args):
    ...
    if args.n_envs == 1 and not FORCE_VEC_ENV:
        ...
        if args.use_RP or args.use_dijk_reward:
        ...
        def create_env():
            e = gym.make('random_rooms-v1', cmd_args=args)
            if args.use_RP or args.use_dijk_reward:
                e = DijkRewardWrapper(e, args)
            return e
    ...
    if args.obs_type == "conv":
        ...
        # model = PPO(policy, env, policy_kwargs=policy_kwargs, verbose=1, n_steps=args.n_steps, tensorboard_log=log_dir)
    ...
    # rp norm callback
    if args.use_RP and args.rp_norm:
    ...
    # def wandb_record(self, *args, **kwargs):
    ...
    # stablebaselines3 update b52c6fc18fa4b48a259c839e8159b6c9f826e8ad
    from stable_baselines3.common.logger import Logger as logger
    orig_record = logger.record
    def wandb_record(self, key, value, exclude=None):
        if 'timesteps' in key:
            wandb_record.step = value
        if 'train' not in key:
            wandb.log({key: value})
        orig_record(self, key, value, exclude)
...
if __name__ == '__main__':
    ...
    RP_group.add_argument('--online_training', type=str, default=None,
                          ...
                          choices=["", 'perfect_user, discrete_user', 'real_user'])
    RP_group.add_argument('--rp_norm', default=False, action='store_true', help='Use RP norm or not use at all')
    # removed args - specify in --online_training instead
    # RP_group.add_argument('--user_reward', default=False, help='Get user input as reward', action='store_true')
    # RP_group.add_argument('--perfect_user', default=False, help='simulate user_Reward with original env reward',
    #                       action='store_true')
    #
